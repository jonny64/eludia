...
### Test 3
$res = eval {
	sql_do('SELECT name id, label label FROM table1 WHERE fake = 0 ORDER BY NULL')
};
error_output($@, 'Syntax AS && ORDER BY NULL');
...
### Test 7
$res = eval {
	sql_do("SELECT GROUP_CONCAT(DISTINCT name) all_name FROM table1 WHERE fake = 0")
};
