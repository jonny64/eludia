...
sub defaults {
	...
	sql_select_loop (
		"SELECT * FROM $conf->{systables}->{__defaults} WHERE fake = 0 AND context = ? AND name IN ($names)",
		sub {
			$def {$i -> {name}} .= $i -> {value};
...
sub check_dbl_click_start {
	...
	return
		if !$preconf -> {core_dbl_click_protection}
			...
			|| $_REQUEST {action} ~~ ['download', 'save_metrics', 'upload_metrics', 'download_metrics', 'get_xml'];
